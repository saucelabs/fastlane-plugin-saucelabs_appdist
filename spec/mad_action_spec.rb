describe Fastlane::Actions::MadAction do
  describe '#run' do
    before do
    ...
    it 'returns the ipa path in test mode' do
      result = Fastlane::FastFile.new.parse("lane :test do
        mad(api_key: 'abc123', ipa: '#{@tmp_ipa}')
    ...
    it 'returns the apk path in test mode' do
      ...
      result = Fastlane::FastFile.new.parse("lane :test do
        mad(api_key: 'abc123', apk: '#{tmp_apk}')
      end").runner.execute(:test)
  ...
  describe '#available_options' do
    it 'has the expected number of options' do
      expect(Fastlane::Actions::MadAction.available_options.length).to eq(18)
    end
  ...
  describe '#is_supported?' do
    it 'supports iOS' do
      expect(Fastlane::Actions::MadAction.is_supported?(:ios)).to be(true)
    end

    it 'supports Android' do
      expect(Fastlane::Actions::MadAction.is_supported?(:android)).to be(true)
    end

    it 'does not support Mac' do
      expect(Fastlane::Actions::MadAction.is_supported?(:mac)).to be(false)
    end
  end
end
